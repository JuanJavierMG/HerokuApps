...
# Define Gunicorn command and start server
if platform.system() == "Windows":
    pass
else:
    # Set Gunicorn configuration variables
    bind_address = f"{socket.gethostbyname(socket.gethostname())}:8050"
    workers = os.cpu_count() * 2 + 1
    timeout = 120

    # Define the command to start Gunicorn
    cmd = f"gunicorn -w {workers} -b {bind_address} -t {timeout} myapp:app"
    ...
    # Initialize Flask-Compress to enable Gzip compression
Compress(app.server)

# Add fcntl module if it is not Windows
if platform.system() != 'Windows':
    try:
        import fcntl
    except ImportError:
